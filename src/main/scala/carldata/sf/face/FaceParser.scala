...
object FaceParser extends RegexParsers {
  ...
  def negativeVariable: Parser[MinusOpExpr] = "-" ~ variable ^^ { id => MinusOpExpr(id._2) }
  def factor: Parser[Expression] = function | number | nullVariable | variable | negativeVariable | "(" ~> addOrBoolExpr <~ ")"
